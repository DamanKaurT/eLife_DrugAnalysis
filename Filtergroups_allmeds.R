...
sub.nacc <- data.frame(NACCID = nacc$NACCID, VISITDAY = nacc$VISITDAY, VISITMO = nacc$VISITMO,
   ...
   APS5 = nacc$DELIR, DIAG = nacc$NACCUDSD, CDR = nacc$CDRSUM, ALZP = nacc$NACCALZP, 
   ALZD = nacc$NACCALZD)
...
## Filter out groups with impairment or MCI at last visit.

## impaired but not MCI
dfe <- dfe %>%
  group_by(NACCID) %>%
  mutate(check1=any(DIAG==2 & NEW_VISITNUM==(n()))) 
dfe <- dfe %>%
  group_by(NACCID) %>%
  filter(check1=="FALSE")
dfe <- dfe[,-(129)] 
...
dfe <- dfe[,-(128)] 
dfe <- dfe[ order(dfe[,1], dfe[,3],dfe[,2]), ]
...
write.csv(df, file = "NtoDudsd.csv",row.names=FALSE, na="")
...
## Now we have healthy to healthy, healthy to impaired, healthy to MCI,
...
df$Diagnosis[(df$Diagnosis == 'TRUE' )]<- 1
df <- df[,-(129)] 
write.csv(df, file = "NtoMudsd.csv",row.names=FALSE, na="")
...
df$Diagnosis[(df$Diagnosis == 'TRUE' )]<- 1
df <- df[,-(129)] 
write.csv(df, file = "MtoDudsd.csv",row.names=FALSE, na="")
